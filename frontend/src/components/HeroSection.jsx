...
const DEFAULT_PROFILE = {
  name: 'Alex',
  about: 'I help modern creators and visionary brands build high-end visual identities, cinematic cuts, and dynamic motion graphics. Let my work speak for itself.',
  taglines: ['Video Editor.', 'Graphic Designer.', 'Visual Storyteller.', 'Motion Artist.'],
  heroImage: '/hero.png',
...
export default function HeroSection() {
  ...
  useEffect(() => {
    api.get('/profile')
      .then(res => setProfile({ ...DEFAULT_PROFILE, ...res.data }))
      .catch(() => { }); // silently fall back to defaults if backend is offline
  }, []);
    ...
    <section className="pt-24 pb-16 md:pt-32 md:pb-24 flex flex-col lg:flex-row items-center gap-12">
      <div className="flex-1 space-y-8 relative z-10 w-full">
          ...
          <h1 className="text-5xl md:text-7xl font-extrabold leading-[1.1]">
            ...
            <span className="text-3xl md:text-4xl lg:text-5xl text-foreground font-semibold mt-4 block h-[60px] md:h-[80px]">
              {profile.taglines?.length > 0 && (
                <Typewriter
                  key={profile.taglines.join(',')} // re-mounts when taglines change
                  options={{
                    strings: profile.taglines,
                    autoStart: true,
                    loop: true,
        ...
        <div className="flex gap-4 items-center pt-2">
          <a href={profile.instagramUrl} target="_blank" rel="noreferrer" className="p-3 glass rounded-full hover:text-primary hover:border-primary/50 transition-colors">
            <Camera className="w-5 h-5" />
          </a>
          <a href={profile.youtubeUrl} target="_blank" rel="noreferrer" className="p-3 glass rounded-full hover:text-primary hover:border-primary/50 transition-colors">
